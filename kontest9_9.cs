    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                endVertex[i] = AddString(s);
                len[i] = s.Length;
            }

            string text = Console.ReadLine();

            int[] minPos = new int[trie.Count];
            int[] maxPos = new int[trie.Count];

            for (int i = 0; i < trie.Count; i++)
            {
                minPos[i] = int.MaxValue;
                maxPos[i] = -1;
            }
            ...
            for (int i = size - 1; i > 0; i--)
            {
                ...
                if (minPos[v] < minPos[p])
                {
                    minPos[p] = minPos[v];
                }

                if (maxPos[v] > maxPos[p])
                {
                    maxPos[p] = maxPos[v];
                }
            }
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    int left = minPos[v] - len[i] + 1;
                    int right = maxPos[v] - len[i] + 1;
